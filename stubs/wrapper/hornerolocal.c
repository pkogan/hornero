...
void enviarSolicitud(char* hostname, char* token, char* problema) {
    ...
    sprintf(request, "GET /hornero/index.php?r=juego/solicitud&token=%s&problema=%s HTTP/1.1\r\nHost: %s\r\n\r\n", token, problema, hostname);
...
void enviarRespuesta(char* hostname, char* respuesta) {
    ...
    sprintf(request, "GET /hornero/index.php?r=juego/respuesta&tokenSolicitud=%s&solucion=%s HTTP/1.1\r\nHost: %s\r\n\r\n", tokenSolicitud, respuesta, hostname);
...
int main(int argc, char *argv[]) 
{
	...
	char *tokenTorneo = argv[3];
        ...
        if(argc != 6) 
                fatal("Mal numero de argumentos: hornero <host> <numProbl> <token> <lenguaje> <prog.exe>");
